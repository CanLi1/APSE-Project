...
from idaes_models.core.process_base import ProcessBase, ProcBlock
...
class FlowsheetModel(ProcessBase):
    ...
    def solve_local_NLP(self, init_strategy='default', tee=False, keepfiles=False, add_min_flows=True, iterations=5):
        ...
        if results.solver.status is SolverStatus.ok and \
            ...
            if(iterations > 1):
                ...
                completed = self.solve_local_NLP(
                    tee=tee, init_strategy=None, keepfiles=keepfiles, add_min_flows=add_min_flows, iterations=iterations - 1)
            return completed
