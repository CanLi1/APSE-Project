...
def add_oa_constraints(iter_num, oa_block, jacs, exists_var, disagg_vars={}):
    ...
    for con_data, jac_info in iteritems(jacs):
        ...
        # verify that all variables have a lower bound, because we subtract by
        # it later
        for var in itervalues(jac_info['_vars_']):
            if getvar(var).lb is None:
                raise ValueError("{} needs to have a lower bound for proper OA cut generation".format(getvar(var).name))
        oa_constr.add(
            indx,
            (None,
