...
from .process_base import ProcessBase, ProcBlock
...
__all__ = ['UnitModel', 'UnitModelBlock']

@ProcBlock("UnitModelBlock")
class UnitModel(ProcessBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._reg_initialize('default', func=self.do_init, doc='Default unit initialization')
            ...
            if condata.body.is_fixed():
                ...
                if condata.upper is None:
                    ...
                    if pe.value(condata.body) > pe.value(condata.upper):
                        raise ValueError('Infeasible constraint: ' + condata.name)
                elif abs(pe.value(condata.body) - pe.value(condata.upper)) > 1E-10 or abs(pe.value(condata.body) - pe.value(condata.lower)) > 1E-10:
