...
class Turbine(unit.UnitModel):
    """
    9. Turbine Unit Class
    
    Model has 2 degree of freedom (with fixed inlet stream).
    Usually set: Ratio_P, Eta (efficiency)
    Usually Calculate: Out_T_Isen, Work_Isen, Work, Out_T
    """
    def __init__(self, comp, main_model=None):
        """
        Create a turbine object with the component set comp.
    ...
    def _make_vars(self, b):
        ...
        b.Eta = Var(domain=NonNegativeReals, bounds=(0,1), initialize=0.9, doc="Effciency with respect to isentropic process")
        b.Out_T_Isen = Var(domain=NonNegativeReals, initialize=273.15, doc="Isentropic outlet temperature (K)")
        b.Work = Var(domain=Reals, initialize=0, doc="Work input to turbine (negative)")
        b.Work_Isen = Var(domain=Reals, initialize=0, doc="Work input to turbine if isentropic process (negative)")
    ...
    def _make_energy_balance(self, b):
        ...
        # Actual work
        b.eq_work = Constraint(expr=b.Work == b.Work_Isen*b.Eta)
