"""
Standard IDAES splitter models. (unreviewed)
...
class Splitter(unit.UnitModel):
    """
    1. Splitter unit class
    """
    ...
    def _make_vars(self, b):
        ...
        b.splitfrac =  Var(self.Nout, domain=NonNegativeReals, 
                              initialize=0.5,
                              doc="Mole fraction of each outlet stream")
...
#        b.sumy = Constraint(self.Nout, rule=rule_sumy)
        ...
        def rule_yfix(blk,i,j):
            if i > 1:
                return blk.Out_y[i,j] == blk.In_y[j]
            if i==1:
                return Constraint.Feasible
        b.yfix = Constraint(self.Nout, self.comp, rule=rule_yfix)

        def rule_sumfrac(blk):
            return sum(blk.splitfrac[i] for i in self.Nout) == 1
        b.sumfrac= Constraint(rule=rule_sumfrac)        

        def rule_split(blk,i):
            if i > 1:
                return blk.Out_F[i] == blk.In_F*blk.splitfrac[i]
            if i == 1:
                return Constraint.Feasible
        b.split= Constraint(self.Nout, rule=rule_split)         
