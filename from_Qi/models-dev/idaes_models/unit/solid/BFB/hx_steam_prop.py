...
class PropPack(unit.UnitModel):
    ...
    def _make_vars(self):
        ...
        blk.vf = Var(initialize=1)
        
        blk.logP = Var(initialize=1)
    ...
    def _make_constraints(self):
        ...
        # Antoine equation (coefficients from NIST)
        blk.eq_P_vap = Constraint(expr = 643.748 == (blk.T-198.043) \
                                              * (3.55959 - blk.logP))
        blk.eq_P = Constraint(expr = 10**blk.logP == blk.P*1e-5)

        # Density constraints
        blk.rho_liq.fix(1)
        blk.eq_rho_vap = Constraint(expr = blk.rho_vap == blk.P/(blk.R*blk.T))

        blk.eq_rho_mix = Constraint(expr = blk.rho_mix == blk.vf*blk.rho_vap \
                                                + (1-blk.vf)*blk.rho_liq)

        # Enthalpy constraints (coefficients from NIST)
        blk.eq_h_liq = Constraint(expr = blk.h_liq == 75.2652*(blk.T-blk.Tref))

        blk.eq_h_mix = Constraint(expr = blk.h_mix == blk.h_liq \
                                                  + blk.vf*40650)