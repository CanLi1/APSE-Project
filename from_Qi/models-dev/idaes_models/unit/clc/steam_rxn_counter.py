...
@ProcBlock("SteamRxnCounter")
class _SteamRxnCounter(UnitModel):

    """
    Equilibrium Counter-current Steam Reactor Model

    """

    def __init__(self, *args, **kwargs):
        """
        Create a flash object with the component set comp and two outlets.
        Args
        ====
        ...
        super(_SteamRxnCounter, self).__init__(*args, **kwargs)
    ...
    def _build_unit_sets(self):
        ...
        b.ignore = Set(initialize=['O2', 'N2', 'CH4', 'CO', 'CO2'])
        # Gas components
    ...
    def _build_unit_vars(self):
        b = self
        b.T = Var(doc='Steam reactor temperature (K)', domain=NonNegativeReals)
        ...
        b.logYeqSolidout = Var(b.comps,
                               doc='Yeq at solid outlet for countercurrent\
                                    reactors')
        b.F = Var(b.k, doc='Steam reactor molar flow rates in mol/s',
                  domain=NonNegativeReals)
        b.p = Var(b.k, b.comps, doc='Partial pressures',
                  domain=NonNegativeReals)
        b.Pt = Var(doc='Steam reactor total pressure', domain=NonNegativeReals)
        b.Hc = Var(b.comps, doc='Standard molar enthalpy of component in J/mol')
        b.H = Var(b.k, doc='Enthalpy of stream in J/s')
        b.Q = Var(doc='Steam reactor heat duty in J/s')
    ...
    def _build_unit_constraints(self):
        ...
        equip.EQ_ox1.deactivate()
        equip.EQ_ox2.deactivate()
        equip.EQ_ox3.deactivate()

        equip.EqYGasIn.deactivate()
        equip.EqYSolidOut1.deactivate()
    ...
    def _build_energy_balance(self):
        ...
        equip.EQ_Hc = Constraint(b.comps, rule=EQ_Hc_rule)
    ...
    def do_init(self):
        ...
        b.y_GasIn['CO'] = 0
        b.y_GasIn['H2'] = 1e-5
        b.y_GasIn['H2O'] = 1 - 1e-5
        b.y_GasIn['O2'] = 0
        b.y_GasIn['N2'] = 0

        '''b.y_SolidIn['Fe2O3'].fix(0.018960676)
        b.y_SolidIn['Fe3O4'].fix(0.094283734)
        b.y_SolidIn['FeO'].fix(0.811632527)
        b.y_SolidIn['Fe'].fix(0.075123061)
        b.y_SolidIn['C'].fix(1e-8)'''

        b.y_SolidIn['Fe2O3'] = 0.018960676
        b.y_SolidIn['Fe3O4'] = 0.094283734
        b.y_SolidIn['FeO'] = 0.811632527
        b.y_SolidIn['Fe'] = 0.075123061
        b.y_SolidIn['C'] = 1e-8
        ...
        for j in b.comps:
            if j not in b.ignore:
                if j in b.gas:
                    b.logYeq[j] = log((value(b.p['GasIn', j])/value(b.Pt)))
                if j in b.solid:
                    b.logYeq[j] = log(value(b.y_SolidIn[j]))
                    b.logYeqSolidout[j] = log(value(b.y_SolidIn[j]))
        ...
        for k in b.k:
        		b.H[k] = sum(value(b.F[k]) * value(getattr(b, 'y_' + k)[j]) * value(b.Hc[j]) for j in b.comps)
...
if __name__ == '__main__':
    ...
    m.units.steam_reactor.solve(tee=True, options={'outlev': 5, 'tol': 1e-8})
