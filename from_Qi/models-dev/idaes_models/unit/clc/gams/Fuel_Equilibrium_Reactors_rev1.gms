...
$Stitle   Fuel Reactors for Chemical Looping
...
Sets    j  Components    / C,CH4,CO,H2,CO2,H2O,Fe2O3,Fe3O4,FeO,Fe,O2,N2 /
        ...
        ignore           / O2,N2 /
        gas              / CH4,CO,H2,CO2,H2O,O2,N2 /
        solid            / C,Fe2O3,Fe3O4,FeO,Fe /
;
...
Variables
         ...
         logYeq(j)       Log of mole fraction used for equilibrium conditions
         logYeqSolidout(j)
...
EqYGasIn(j)$(not ignore(j) and gas(j)) .. logYeq(j) + log(F('GasIn'))
         =e= log(y('GasIn',j)*F('GasIn'));
EqYGasOut(j)$(not ignore(j) and gas(j)) .. logYeq(j) + log(F('GasOut'))
         =e= log(y('GasOut',j)*F('GasOut'));
...
obj .. objective =g= 1;
obj1 .. objective =e= -y('SolidOut','Fe3O4');
...
****************************************************************************
****************************************************************************
* Concurrent Reducer Reactor
Model  EQreducer_concur  /
* EQ_rxn4
         EQ_rxn5
         EQ_rxn6
         EQ_rxn7
...
* Example inlet conditions for reducer reactor
         y.fx('GasIn','CH4') = 1-4e-5;
         y.fx('GasIn','CO2') = 1e-5;
         y.fx('GasIn','CO') = 1e-5;
         y.fx('GasIn','H2') = 1e-5;
         y.fx('GasIn','H2O') = 1e-5;
         ...
         y.fx('SolidIn','Fe2O3') = 1-4e-5;
         y.fx('SolidIn','Fe3O4') = 1e-5;
         y.fx('SolidIn','FeO') = 1e-5;
         y.fx('SolidIn','Fe') = 1e-5;
         y.fx('SolidIn','C') = 1e-5;
         ...
         logYeq.l(j)$(not ignore(j)) = log((y.l('GasIn',j)+y.l('SolidIn',j)));

         EQreducer_concur.optfile = 1;
         solve EQreducer_concur using nlp  minimizing  objective;
         statstore(iter) = EQreducer_concur.modelstat;
         loop(j,
                 loop(k,
...
****************************************************************************
****************************************************************************
* Countercurrent Reducer Reactor
Model  EQreducer_counter  /
         EQ_rxn4
* EQ_rxn5
         EQ_rxn6
* EQ_rxn7
...
* Example inlet conditions for reducer reactor
         ...
         EQreducer_counter.optfile = 1;
         solve EQreducer_counter using nlp  minimizing  objective;
         statstore(iter) = EQreducer_counter.modelstat;
         loop(j,
                 loop(k,
                         Fc(k,j,iter) = y.l(k,j)*F.l(k);
                         ystore(k,j,iter) = y.l(k,j);
