...
$Stitle   Air Reactors for Chemical Looping
...
Sets    j  Components    / C,CH4,CO,H2,CO2,H2O,Fe2O3,Fe3O4,FeO,Fe,O2,N2 /
        ...
        ignore(j) Not included in EQ calcs - may be zero /H2,H2O,N2,CH4,CO,CO2/
...
* Example inlet conditions for air reactor
         ...
         EQair_concur.optfile = 1;
         solve EQair_concur using nlp  minimizing  objective;
         statstore(iter) = EQair_concur.modelstat;
         loop(j,
                 loop(k,
...
* Example inlet conditions for air reactor
         ...
         EQair_counter.optfile = 1;
         solve EQair_counter using nlp  minimizing  objective;
         statstore(iter) = EQair_counter.modelstat;
         loop(j,
                 loop(k,
                         Fc(k,j,iter) = y.l(k,j)*F.l(k);
                         ystore(k,j,iter) = y.l(k,j);
